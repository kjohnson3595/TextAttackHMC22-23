...
class ModelArgs:
    ...
    def _create_model_from_args(cls, args):
        ...
        if args.model_from_file:
            ...
            # is instantiated.
            colored_model_name = eukaryote.shared.utils.color_text(
                args.model_from_file, color="blue", method="ansi"
            )
            eukaryote.shared.logger.info(
            ...
            if not isinstance(model, eukaryote.models.wrappers.ModelWrapper):
                raise TypeError(
            ...
            if args.model.startswith("lstm"):
                ...
                model = eukaryote.models.helpers.LSTMForClassification.from_pretrained(
                    args.model
                )
                ...
                model = (
                    eukaryote.models.helpers.WordCNNForClassification.from_pretrained(
                        args.model
                    )
                )
            elif args.model.startswith("t5"):
                model = eukaryote.models.helpers.T5ForTextToText.from_pretrained(
                    args.model
                )
            else:
            ...
            # a HuggingFace model).
            if isinstance(model, eukaryote.models.helpers.T5ForTextToText):
                model = eukaryote.models.wrappers.HuggingFaceModelWrapper(
                    model, model.tokenizer
                )
            else:
                model = eukaryote.models.wrappers.PyTorchModelWrapper(
                    model, model.tokenizer
                )
        ...
        assert isinstance(
            model, eukaryote.models.wrappers.ModelWrapper
